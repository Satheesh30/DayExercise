...
public class OptimisticLock {
	...
	public static void main(String[] args) {
		...
		Thread subramain=new Thread(new CounsellingJob(university),"subramain");
		Thread josh=new Thread(new CounsellingJob(university),"josh");
		subramain.start();
		josh.start();
	}
}
class CounsellingJob implements Runnable{
	...
	//	synchronized (university) {
		...
		if(Thread.currentThread().getName().equals("subramain")) {
			university.table1();
			university.table2();
		}
		else if(Thread.currentThread().getName().equals("josh")){
			university.water();
		}
	//	}
	}
